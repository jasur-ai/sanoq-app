...
function normalizeDashboardPrefs(value) {
  ...
  return {
    ...defaults,
    ...(value || {}),
    order: order.filter((id) => defaults.order.includes(id)),
    widgets: Array.isArray(value?.widgets) ? value.widgets.filter((id) => defaults.order.includes(id)) : [...defaults.widgets],
    hideStats: Boolean(value?.hideStats),
  };
}
...
function loadStats(username) {
  ...
  const stats = {
    ...
    preferences: normalizeDashboardPrefs(base.preferences),
...
function AuthScreen({ onAuth, theme, onThemeToggle }) {
  ...
  const signInWithGoogle = async () => {
    if (!supabaseConfigured || !supabase) return setError('Online server sozlanmagan.');
    ...
    if (googleError) setError(googleError.message || 'Google orqali kirishda xatolik yuz berdi.');
    setIsBusy(false);
  };
  const submit = async (event) => {
    ...
    try {
      if (mode === 'reset') {
        ...
        const { error: resetError } = await supabase.auth.resetPasswordForEmail(form.email.trim().toLowerCase(), { redirectTo: window.location.origin });
        if (resetError) throw resetError;
        setInfo('Parolni tiklash havolasi emailingizga yuborildi.');
        return;
      }
      const username = normalizeUsername(form.username);
      if (!/^[a-z0-9_]{3,24}$/.test(username)) throw new Error('Username 3–24 belgidan iborat bo‘lsin: harf, raqam yoki _.');
      if (form.password.length < 6) throw new Error('Parol kamida 6 belgidan iborat bo‘lsin.');
      if (mode === 'signup') {
      ...
      const message = String(authError?.message || 'Amalni bajarishda xatolik yuz berdi.');
      if (message.toLowerCase().includes('already registered')) setError('Bu email allaqachon ro‘yxatdan o‘tgan.');
...
function Modal({ modal, onClose, onConfirm, manualValue, setManualValue }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className={`modal-card ${isManual ? 'manual-modal' : ''}`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose} aria-label="Yopish"><Icon name="x" size={19} /></button><div className={`modal-icon ${isReset ? 'danger-icon' : ''}`}><Icon name={isReset ? 'refresh' : isManual ? 'edit' : 'plus'} size={24} /></div><h2>{isReset ? 'Manual sanog‘ini tiklaysizmi?' : isManual ? `${modal.prayer.title} sanog‘i` : `${modal.prayer.title} ga qo‘shamizmi?`}</h2><p>{isReset ? 'Tasdiqlasangiz, manual rejimdan oldingi sanoqlar va bugungi holat qaytariladi.' : isManual ? 'Istalgan sonni kiriting. Tasdiqlangach, yangi sanoq saqlanadi.' : `Tasdiqlasangiz, ${modal.prayer.title} sanog‘i 1 taga oshadi.`}</p>{isManual && <label className="manual-input-label">Yangi sanoq<span className="manual-input-wrap"><input ref={inputRef} type="number" min="0" inputMode="numeric" value={manualValue} onChange={(event) => setManualValue(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && onConfirm()} /><span>marta</span></span></label>}<div className="modal-actions"><button className="secondary-button" onClick={onClose}>Bekor qilish</button><button className={`primary-button ${isReset ? 'danger-button' : ''}`} onClick={onConfirm}>{isReset ? 'Ha, tiklash' : isManual ? 'Saqlash' : 'Ha, qo‘shish'}{!isManual && !isReset && <Icon name="check" size={17} />}</button></div></section></div>;
}
...
function QuranView({ progress, onChange, onSaveSession }) {
  ...
  if (activeSurah) return <QuranReader surah={activeSurah} progress={progress} mode={mode} onChange={(next) => { setMode(next.lastPosition.mode); setReaderView(next.readerPrefs?.view || readerView); onChange(next); }} onSaveSession={onSaveSession} onBack={() => setActiveSurahId(null)} />;
...
function StatsPreview({ stats, onOpen, onToggleVisibility }) {
  const total = Object.values(stats.counts).reduce((sum, value) => sum + Number(value || 0), 0);
  const todayDone = PRAYERS.filter((prayer) => Number(stats.daily[prayer.id] || 0) > 0).length;
  const quranMinutes = stats.quran.sessions.reduce((sum, item) => sum + Number(item.minutes || 0), 0);
  const hidden = Boolean(stats.preferences.hideStats);
  const display = (value) => hidden ? '••••' : value;
  return <section className="dashboard-widget stats-preview-widget"><div className="widget-heading"><div><div className="eyebrow dark"><span className="eyebrow-dot" /> Natijalar</div><h2>Statistika qisqacha</h2></div><div className="widget-heading-actions"><button className="privacy-toggle" onClick={onToggleVisibility} title={hidden ? 'Raqamlarni ko‘rsatish' : 'Raqamlarni yashirish'}><Icon name={hidden ? 'eyeOff' : 'eye'} size={15} /></button><button className="text-button" onClick={onOpen}>To‘liq statistika <Icon name="arrowRight" size={14} /></button></div></div><div className="preview-metrics"><div><span>Bugungi namoz</span><strong>{display(`${todayDone}/6`)}</strong></div><div><span>Jami sanoq</span><strong>{display(formatNumber(total))}</strong></div><div><span>Qur’on vaqti</span><strong>{display(`${quranMinutes} daq`)}</strong></div></div></section>;
}
...
function CustomizeModal({ prefs, onClose, onSave }) {
  const [draft, setDraft] = useState({ widgets: [...prefs.widgets], order: [...prefs.order], hideStats: Boolean(prefs.hideStats) });
  ...
  const dropOn = (targetId) => setDraft((current) => { if (!draggingId || draggingId === targetId) return current; const order = current.order.filter((id) => id !== draggingId); const targetIndex = order.indexOf(targetId); order.splice(targetIndex < 0 ? order.length : targetIndex, 0, draggingId); return { ...current, order }; });
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="customize-modal" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose}><Icon name="x" size={19} /></button><div className="modal-icon"><Icon name="settings" size={23} /></div><h2>Bosh sahifani sozlash</h2><p>Bo‘limlarni yoqing yoki o‘chiring. Tartibni tugmalar bilan yoki sudrab joylashtiring.</p><div className="customize-list">{draft.order.map((id, index) => <div className={`customize-row ${draggingId === id ? 'dragging' : ''}`} key={id} draggable onDragStart={() => setDraggingId(id)} onDragEnd={() => setDraggingId(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropOn(id)}><span className="drag-handle" title="Sudrab tartiblang">⠿</span><button className={`customize-check ${draft.widgets.includes(id) ? 'selected' : ''}`} onClick={() => toggle(id)}><span>{draft.widgets.includes(id) && <Icon name="check" size={14} />}</span><strong>{labels[id]}</strong></button><div className="order-buttons"><button disabled={index === 0} onClick={() => move(id, -1)} aria-label="Yuqoriga">↑</button><button disabled={index === draft.order.length - 1} onClick={() => move(id, 1)} aria-label="Pastga">↓</button></div></div>)}</div><label className="privacy-setting"><span><Icon name={draft.hideStats ? 'eyeOff' : 'eye'} size={15} /><b>Dashboard statistikasi raqamlarini yashirish</b></span><input type="checkbox" checked={draft.hideStats} onChange={(event) => setDraft((current) => ({ ...current, hideStats: event.target.checked }))} /></label><div className="modal-actions"><button className="secondary-button" onClick={onClose}>Bekor qilish</button><button className="primary-button" onClick={() => onSave(draft)}>Saqlash</button></div></section></div>;
}
...
function Dashboard({ session, onLogout, theme, onThemeToggle }) {
  ...
  const savePrefs = (prefs) => { persistStats({ ...stats, preferences: normalizeDashboardPrefs({ widgets: prefs.widgets, order: prefs.order, hideStats: prefs.hideStats }) }); setCustomizeOpen(false); showToast('Bosh sahifa sozlamalari saqlandi.'); };
...
function UpdatePasswordScreen({ theme, onThemeToggle, onDone }) {
  ...
  const submit = async (event) => {
    ...
    if (updateError) setError(updateError.message || 'Parolni yangilashda xatolik yuz berdi.');
    else { setInfo('Parol yangilandi. Endi yangi parol bilan kiring.'); window.setTimeout(onDone, 900); }
    setBusy(false);
  };
  return <main className="auth-page"><header className="auth-topbar"><Brand /><button className="icon-button ghost" onClick={onThemeToggle} aria-label="Rejimni almashtirish"><Icon name={theme === 'light' ? 'moon' : 'sun'} size={19} /></button></header><section className="auth-layout recovery-layout"><div className="auth-card"><div className="auth-card-heading"><div className="mobile-brand"><Brand compact /></div><h2>Yangi parol</h2><p>Hisobingiz uchun yangi parolni belgilang.</p></div><form onSubmit={submit} className="auth-form"><label className="field-label">Yangi parol<span className="input-wrap"><Icon name="lock" size={18} /><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="••••••••" /></span></label><label className="field-label">Parolni takrorlang<span className="input-wrap"><Icon name="lock" size={18} /><input type="password" value={repeat} onChange={(event) => setRepeat(event.target.value)} placeholder="••••••••" /></span></label>{error && <div className="form-error"><Icon name="info" size={16} />{error}</div>}{info && <div className="form-info"><Icon name="checkCircle" size={16} />{info}</div>}<button className="primary-button auth-submit" disabled={busy}>{busy ? <span className="button-loader" /> : <>Parolni saqlash <Icon name="check" size={17} /></>}</button></form></div></section></main>;
}
...
function App() {
  ...
  if (recoveryMode) return <UpdatePasswordScreen theme={theme} onThemeToggle={toggleTheme} onDone={() => setRecoveryMode(false)} />;
